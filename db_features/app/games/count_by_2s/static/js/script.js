// app/games/count_by_2s/static/js/script.js
(() => {
  'use strict';
  ...
  function resetStats(){ stats.played=stats.solved=stats.revealed=stats.skipped=0; stats.totalTime=0; updateStatsUI(); }
  ...
  // ---------- Session state ----------
  let current = null;
  let uiQ = 0;                 // 0-based; show Q{uiQ+1}
  let countedThisPuzzle = false;

  // timers
  let tStart=0, tTick=null, nextTimer=null;
  ...
  function timerStart(){
    timerStop(); tStart = performance.now();
    if (el.timer) el.timer.textContent = '00:00.0';
    tTick = setInterval(()=>{ if (el.timer) el.timer.textContent = fmtTimer(performance.now()-tStart); }, 100);
  }
  ...
  function addElapsedToTotal(){
    ...
    stats.totalTime += secs; tStart = 0; updateStatsUI();
  }

  function cancelNextDeal(){ if (nextTimer){ clearTimeout(nextTimer); nextTimer=null; } }
  function scheduleNextDeal(){ cancelNextDeal(); if (autoDealOn){ nextTimer = setTimeout(()=>{ nextTimer=null; deal(); }, 900); } }
  ...
  function showModal(id){ const m = (typeof id==='string')?document.getElementById(id):id; if (m) m.style.display='flex'; }
  function hideModal(id){ const m = (typeof id==='string')?document.getElementById(id):id; if (m) m.style.display='none'; }
  ...
  async function check(){
    ...
    try{
      ...
      if (res.ok){
        stats.solved += 1;
        addElapsedToTotal(); updateStatsUI();
  ...
  async function doRestart(){
    ...
    resetStats(); uiQ = 0;
    try{ await fetchJSON(`${API_BASE}/restart`, { method:'POST' }); }catch(_){}
    deal();
  }
  ...
  function showSummaryAndGo(statsObj, url){
    if (el.summaryBackdrop && el.summaryBody && el.summaryOk && el.summaryClose){
      ...
      overlay.innerHTML = `<div style="background:#fff;padding:20px 24px;border-radius:12px;max-width:420px">
        <h3>Session Summary</h3>${renderSummary(statsObj)}
        <div style="text-align:right"><button id="__sum_ok">OK</button></div></div>`;
