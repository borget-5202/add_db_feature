# app/games/count_by_2s/logic/puzzle_store.py
...
# Adjustable cap (env or Flask config can override later)
...
class CB2SPuzzle:
    ...
    external_id: str
    difficulty: str
    cards: List[int]  # e.g. [base, 2, 2, 2]

_store: Dict[str, Any] | None = None

def init_store(force: bool = False, cap: Optional[int] = None) -> None:
    """
    Load up-to 'cap' puzzles per difficulty into memory.
    Builds an 'all' pool too. Safe to call multiple times.
    """
    global _store
    ...
    for r in rows:
        ...
        if not isinstance(cards, list) or len(cards) != 4:
            # Skip malformed rows; we expect [base, 2, 2, 2]
            continue
        p = CB2SPuzzle(
            id=int(r["id"]),
...
def random_next(level: str, avoid_ids: Optional[set[int]] = None) -> Tuple[Optional[CB2SPuzzle], bool]:
    """
    Return a random puzzle for level (easy|medium|hard|all),
    avoiding ids in avoid_ids. If pool is exhausted, return
    from full pool and mark pool_done=True (so caller can reset seen).
    """
    _ensure_loaded()
    lvl = level.lower()
    ...
    pool_done = False
    if not candidates:
        # exhausted -> allow repeats this time
        candidates = pool[:]
        pool_done = True
...
def expected_final(cards: List[int]) -> int:
    """
    For puzzles shaped like [base, 2, 2, 2], the final is base + 2 + 2 + 2.
    """
