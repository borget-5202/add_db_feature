from .database_helpers import get_game_data, save_game_data
import uuid
import json
...
ASSETS_DIR = BASE_DIR / "game24" / "static" / "game24" / "assets" / "images" / "classic"

ANSWERS_PATH = BASE_DIR / "game24" / "static" / "game24" / "answers.json"
...
def init_shared_state():
    ...
    print(f"~~~I loaded {len(ALL_PUZZLES)} puzzles ")
    ...
    print(f"~~~I loaded PUZZLES_BY_ID {len(PUZZLES_BY_ID)} puzzles ")
    print(f"~~~I loaded PUZZLES_BY_KEY {len(PUZZLES_BY_KEY)} puzzles ")
...
def pre_process_pool(puzzles):
    ...
    print("Pre-processing puzzles into difficulty pools")
    print(f"~~~Isee puzzles : {len(puzzles)}")
    print("First 3 puzzles: %s", puzzles[:3] if puzzles else "EMPTY")
    ...
    print(f"~~~easy_like = {len(easy_like)}, hard_like={len(hard_like)}")
