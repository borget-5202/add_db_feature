...
window.fetch = async function (input, init = {}) {
  try {
    ...
    // Normalize URL to a pathname (works for absolute/relative)
    const href = (typeof input === "string") ? input : (input && input.url) || "";
    const u = new URL(href, window.location.origin);
    const pathname = u.pathname;

    // Build API matchers
    const injectedBase = (typeof window !== "undefined" && window.GAME24_API_BASE)
        ? window.GAME24_API_BASE
        : "/game24/api";
    const apiPath = new URL(injectedBase, window.location.origin).pathname;

    const isApi = pathname.startsWith(apiPath) || pathname.startsWith(LEGACY_PATH);
...
function handleNewPuzzleData(data) {
  // basic state
  currentSeq = (data && typeof data.seq === 'number') ? data.seq : 0;
  current = data || {};

  // debug payload shape
  try { console.log("NEXT payload:", JSON.parse(JSON.stringify(data))); }
  catch { console.log("NEXT payload:", data); }
  ...
  if (typeof data.time_left === 'number') {
    startCompetitionCountdown(data.time_left);
  } else {
    stopCompetitionCountdown();
  }
  competitionOver = false;
  ...
  if (el.question) {
    ...
    el.question.textContent = `Q${currentSeq} [#${data.case_id ?? ''}] — Cards: ${qText}`;
  }
  ...
  if (el.cards) {
    ...
    // ---------- FALLBACK: ensure data.images exists ----------
    if (!Array.isArray(data.images) || data.images.length === 0) {
      const staticBase = window.GAME24_STATIC_BASE; // e.g. /game24/static/game24
      const suits = ['C','D','H','S'];
      const rankToken = (v) => {
        const n = Number(v);
        if (!Number.isNaN(n)) return ({1:'A',10:'T',11:'J',12:'Q',13:'K'}[n] || String(n));
        return String(v); // already 'A','T','J','Q','K' or '2'..'9'
      };

      let derived = [];

      // Case 1: data.codes like ["TC","AS",...]
      if (Array.isArray(data.codes) && data.codes.length) {
        derived = data.codes.slice(0,4).map(cd => ({
          code: cd,
          url: `${staticBase}/assets/images/classic/${cd}.png`
        }));
      }

      // Case 2: data.cards like [{rank:10,suit:"C"}, ...] or [[10,"C"],...]
      if ((!derived.length) && Array.isArray(data.cards) && data.cards.length) {
        derived = data.cards.slice(0,4).map(c => {
          let rank, suit;
          if (Array.isArray(c)) { [rank, suit] = c; }
          else if (c && typeof c === 'object') { rank = c.rank; suit = c.suit; }
          if (rank == null || !suit) return null;
          const rt = rankToken(rank);
          const code = `${rt}${suit}`;
          return { code, url: `${staticBase}/assets/images/classic/${code}.png` };
        }).filter(Boolean);
      }

      // Case 3: data.question like [5,10,1,13] → assume suits C/D/H/S
      if ((!derived.length) && Array.isArray(data.question) && data.question.length) {
        derived = data.question.slice(0,4).map((r,i) => {
          const rt = rankToken(r);
      ...
      data.images = derived; // may be empty if nothing matched
    }
    // ---------- END FALLBACK ----------

    // render cards or a small message
    if (!Array.isArray(data.images) || data.images.length === 0) {
      const msg = document.createElement('div');
      msg.className = 'status status-warning';
      msg.textContent = 'No images available for this puzzle.';
      el.cards.appendChild(msg);
    } else {
...
// static/js/script.js - Fix competition handling
  async function deal() {
      ...
      try {
          const r = await fetch(`${API_BASE}/next?theme=${encodeURIComponent(themeVal)}&level=${encodeURIComponent(levelVal)}&seq=${currentSeq}&client_id=${encodeURIComponent(CLIENT_ID)}&guest_id=${encodeURIComponent(GUEST_ID)}`);
   ...
   async function showPoolReportNow(){
     try{
       const r = await fetch('${API_BASE}/pool_report');
  ...
  async function loadCaseById() {
    ...
    try {
      const themeVal = safeValue(el.theme, 'classic');
      const levelVal = safeValue(el.level, 'easy');
      const r = await fetch(`${API_BASE}/next?theme=${encodeURIComponent(themeVal)}&level=${encodeURIComponent(levelVal)}&case_id=${caseId}&seq=${currentSeq}&client_id=${encodeURIComponent(CLIENT_ID)}&guest_id=${encodeURIComponent(GUEST_ID)}`);
      if (!r.ok) {
  ...
  async function exitGame(hard = true) {
    try {
      const r = await fetch(`${API_BASE}/exit?hard=${hard ? 'true' : 'false'}`, { method: 'POST' });
      const data = await r.json();
      if (data.ok && hard && data.next_url) {
        window.location.href = data.next_url;   // ← navigate to home
      } else if (data.ok) {
        // soft exit: pool cleared, stay on page
        deal();
      } else {
        console.warn('Exit failed:', data);
      }
    } catch (e) {
  ...
  async function restartGame() {
    ...
    const data = await r.json();
    if (data.ok) {
      // reset UI counters locally if you keep local mirrors
      stats = { played:0, solved:0, revealed:0, skipped:0 }; // etc...
      updateStats();
      // optionally auto-deal a fresh hand
      deal();
    }
  }
  ...
  on(el.exit, 'click', (e)=>{
      ...
      if(!confirm('Exit the session and see your summary?')) {
        console.log('User cancelled exit'); // Debug log
        return; // This should stop execution
      }
      console.log('User confirmed exit'); // Debug log
      addToTotalTime();
      stopCompetitionCountdown();
      competitionOver = false;
      window.exitAndShowSummary();  // Use the global function that already works
  });
...
(function initSummaryUI(){
  ...
  function hideSummaryModal(redirect = true) {
...
/* ---- Force client_id / guest_id onto ALL game API requests ---- */
(function patchFetchForClientIds() {
  // Ensure stable IDs in storage
  try {
    window.CLIENT_ID = window.CLIENT_ID
      || sessionStorage.getItem('client_id')
      || (crypto && crypto.randomUUID ? crypto.randomUUID() : String(Math.random()));
  ...
  // Helpers
  const toUrlStr = (input) => (input instanceof Request ? input.url : String(input));
  const normPath = (href) => new URL(href, window.location.origin).pathname;

  // Build API matchers
  const injectedBase = (typeof window !== "undefined" && window.GAME24_API_BASE)
    ? window.GAME24_API_BASE
    : "/game24/api"; // legacy fallback
  const apiPath = normPath(injectedBase);                 // e.g., "/game24/api"
  const isGameApi = (pathname) =>
    pathname.startsWith(apiPath) || pathname.startsWith(LEGACY_PATH); // legacy

  window.fetch = function (input, init = {}) {
    try {
      let url = (typeof input === "string") ? input : toUrlStr(input);
      const pathname = normPath(url);

      // Only touch our game API calls
      if (isGameApi(pathname)) {
        ...
        if (method === "GET") {
          ...
          // keep input relative (pathname + search)
          url = u.pathname + u.search;
          input = url;
        } else {
          // Merge JSON body (don’t break existing headers)
...
/* --- Force IDs & sync footer with server stats on every game API call --- */
(function patchFetchAndSyncCounters() {
  ...
  // API bases (injected + legacy fallback)
  const injectedBase = (typeof window !== "undefined" && window.GAME24_API_BASE)
    ? window.GAME24_API_BASE
    : "/game24/api";               // cononical
  const API_PATH = normPath(injectedBase);      // e.g. "/game24/api"
  const LEGACY_PATH = "/api/game24";

  const isGameApiPath = (pathname) =>
    pathname.startsWith(API_PATH) || pathname.startsWith(LEGACY_PATH);

  const nextPaths = [API_PATH + "/next", LEGACY_PATH + "/next"];
  ...
  window.fetch = function (input, init = {}) {
    ...
    try {
      ...
      if (isGameApiPath(pathname)) {
        ...
        if (method === "GET") {
          ...
          url = u.pathname + u.search;   // keep relative
          input = url;
        } else {
          init.headers = Object.assign({'Content-Type':'application/json'}, init.headers || {});
          let body = {};
    ...
    return origFetch(input, init).then((resp) => {
      try {
        const pathname = normPath((typeof input === 'string') ? input : toUrlStr(input));
        if (!isGameApiPath(pathname)) return resp;

        // when a new hand is dealt successfully, clear local "hand interacted"
        if (resp.ok && nextPaths.some(p => pathname.startsWith(p))) {
...
(function fixCountersToServerTruth(){
  ...
  async function refetchStats() {
    try {
      const r = await fetch(`${API_BASE}/pool_report?client_id=${encodeURIComponent(window.CLIENT_ID)}&guest_id=${encodeURIComponent(window.GUEST_ID)}`);
      if (!r.ok) return;
      const j = await r.json();
...
(function () {
  ...
  // Build dynamic matchers from injected base (e.g., "/game24/api")
  const injectedBase = (typeof window !== "undefined" && window.GAME24_API_BASE) ? window.GAME24_API_BASE : "/game24/api";
  const API_PATH = pathOf(injectedBase); // normalized pathname
  const apiPrefixRe = new RegExp("^" + reEscape(API_PATH) + "(?:/|$)");

  // Backward compatibility with legacy URLs
  const legacyApiPrefixRe = /^\/api\/game24(?:\/|$)/;

  // For fallback stats refresh on specific endpoints
  const checkHelpRe = new RegExp("^" + reEscape(API_PATH) + "/(check|help)(?:/|$)");
  const legacyCheckHelpRe = /^\/api\/game24\/(check|help)(?:\/|$)/;

  window.fetch = function (input, init = {}) {
    let url = (typeof input === "string") ? input : toUrlStr(input);
    try {
      const pathname = pathOf(url);
      const isApi = apiPrefixRe.test(pathname) || legacyApiPrefixRe.test(pathname);
      ...
      if (isApi) {
        ...
        if (method === "GET") {
          ...
          if (!u.searchParams.has("client_id")) u.searchParams.set("client_id", window.CLIENT_ID);
          if (!u.searchParams.has("guest_id"))  u.searchParams.set("guest_id",  window.GUEST_ID);
          url = u.pathname + u.search;   // keep it relative
    ...
    try {
      const pathname = pathOf((typeof input === "string") ? input : toUrlStr(input));
      const isApi = apiPrefixRe.test(pathname) || legacyApiPrefixRe.test(pathname);
      ...
      if (isApi) {
        p.then((resp) => {
          ...
          if (ctype.includes("application/json")) {
            resp.clone().json().then((j) => {
              if (j && j.stats) {
                setCountersFromStats(j.stats);     // Prefer server-returned stats
              } else if (checkHelpRe.test(pathname) || legacyCheckHelpRe.test(pathname)) {
                // Fallback: re-pull stats if /check or /help didn't include them
...
(function patchXHRforClientIds() {
  ...
  // Helpers
  const normPath = (href) => new URL(String(href), window.location.origin).pathname;

  // API bases (injected + legacy fallback)
  const injectedBase = (typeof window !== "undefined" && window.GAME24_API_BASE)
    ? window.GAME24_API_BASE
    : "/game24/api";                            // canonical
  const API_PATH    = normPath(injectedBase);   // e.g. "/game24/api"
  const LEGACY_PATH = "/api/game24";
  ...
  // Intercept open() so we can actually modify the request URL before send()
  XMLHttpRequest.prototype.open = function(method, url, async, user, password) {
    try {
      const href = String(url);
      const pathname = normPath(href);
      const isApi = isGameApiPath(pathname);
      this.__isApi = !!isApi;

      if (isApi) {
        // Add IDs to the query string up front
        const u = new URL(href, window.location.origin);
        if (!u.searchParams.has('client_id')) u.searchParams.set('client_id', window.CLIENT_ID);
        if (!u.searchParams.has('guest_id'))  u.searchParams.set('guest_id',  window.GUEST_ID);

        // Keep URL relative (pathname + search) so it matches fetch behavior
        const newUrl = u.pathname + u.search;

        // Replace the 'url' argument and call through
        arguments[1] = newUrl;
        this.__apiUrl = newUrl;          // remember for send()
        this.__apiPathname = u.pathname; // for quick checks later if needed
      } else {
        this.__apiUrl = href;
        this.__apiPathname = pathname;
      }
    } catch(e) {
      // If anything goes wrong, fall back to original values
      this.__apiUrl = url;
      this.__apiPathname = (url ? String(url) : "");
    }
    return XHROpen.apply(this, arguments);
  };

  // Intercept send() so we can inject IDs into POST/PUT bodies
  XMLHttpRequest.prototype.send = function(body) {
    try {
      if (this.__isApi) {
        // Inject IDs into body for JSON / form encodings / FormData
        if (body instanceof FormData) {
          ...
          // Ensure Content-Type for JSON case (best-effort; setRequestHeader may be used elsewhere)
          if (updated && body.trim().startsWith('{')) {
